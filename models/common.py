...
class DeformConv2dPack(nn.Module):
    def __init__(self,
        ...
        #     c1,
        #     g * self.kernel_size[0] * self.kernel_size[1],
        #     kernel_size=k,
        #     stride=_pair(s),
        #     padding=_pair(p),
        #     bias=True)

        self.deformable = DeformConv2d(in_channels=c1, out_channels=c2, kernel_size=k, stride=s, padding=p,bias=False)
        self.bn = nn.BatchNorm2d(c2)
        self.act = nn.SiLU() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
        self.init_offset()
    ...
    def init_offset(self):
        ...
        # self.modulator_conv.weight.data.zero_()
        # self.modulator_conv.bias.data.zero_()

    def forward(self, x):
        offset = self.conv_offset(x)
        # modulator = self.modulator_conv(x)
        return self.act(self.bn(self.deformable(x, offset)))
##### end of DU-MobuleYOLO ####
